...
impl TradeExecutor {
    ...
    async fn execute_swap(&self, params: TradeParams) -> Result<TradeResult> {
        ...
        let price_impact = quote["priceImpactPct"].as_str()
            .unwrap_or("0")
            .parse::<f64>()
            .unwrap_or(0.0);
    ...
    pub async fn get_estimated_price_lamports_per_token(&self, mint: &Pubkey, amount_tokens: u64) -> Result<f64> {
        ...
        let quote = self.get_jupiter_quote(&params).await?;
        let out_amount = quote["outAmount"].as_str().unwrap_or("0").parse::<u64>().unwrap_or(0);
    ...
    async fn get_swap_transaction(&self, quote: &Value, params: &TradeParams) -> Result<Transaction> {
