...
#[derive(Parser, Debug)]
struct Args {
    #[arg(long)] input: String,           // trades_last7.csv lub .json
    #[arg(long)] output: String,          // optimized.yaml
    #[arg(long, default_value = "csv")] format: String, // csv|json
    #[arg(long)] api_url: Option<String>, // http://127.0.0.1:3001
    #[arg(long)] auth_token: Option<String>,
    #[arg(long, default_value_t = true)] dry_run: bool,
    #[arg(long, default_value_t = 30)] generations: usize,
    #[arg(long, default_value_t = 30)] population: usize,
}
...
#[derive(Serialize)]
struct ExecOut { priority_fee_lamports: u64, max_retries: u32 }
...
fn evolve(mut pop: Vec<Gene>, rows: &[RowCsv], generations: usize) -> Gene {
    ...
    for _ in 0..generations {
        for g in pop.iter_mut() { g.fitness = eval_fitness(g, rows); }
        pop.sort_by(|a,b| b.fitness.partial_cmp(&a.fitness).unwrap());
        let elite = pop[0].clone();
        // mutacje lekkie
        for g in pop.iter_mut().skip(1).take(5) {
            g.min_profit_percent = (g.min_profit_percent + rng.gen_range(-0.05..0.05)).clamp(0.05, 1.0);
            g.max_slippage_percent = (g.max_slippage_percent + rng.gen_range(-0.1..0.1)).clamp(0.1, 1.5);
    ...
    pop.into_iter().max_by(|a,b| a.fitness.partial_cmp(&b.fitness).unwrap()).unwrap()
}
...
async fn main() -> Result<()> {
    ...
    if rows.is_empty() { return Err(anyhow!("No data rows")); }

    // inicjalna populacja
    let mut rng = rand::thread_rng();
    let pop = (0..args.population).map(|_| Gene {
    ...
    let out = OutputYaml {
        ...
        execution: ExecOut { priority_fee_lamports: 5000, max_retries: 3 },
    };
    ...
    if !args.dry_run {
        if let Some(api) = args.api_url {
            ...
            if let Some(tok) = args.auth_token { req = req.header("Authorization", format!("Bearer {}", tok)); }
