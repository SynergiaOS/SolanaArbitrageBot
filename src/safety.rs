...
impl SafetyGuard {
    ...
    pub async fn should_continue_trading(&self, state: &crate::SharedState) -> bool {
        ...
        if trades_today >= max_daily_trades {
            warn!("⛔ Daily trade limit reached: {}/{}", trades_today, max_daily_trades);
            return false;
        }
    ...
    pub async fn health_check(&self) -> Result<bool> {
        ...
        let history_ok = {
            ...
            let failure_rate = if !recent_trades.is_empty() {
                recent_trades.iter().filter(|t| !t.success).count() as f64 / recent_trades.len() as f64
            } else {
                0.0
            };
...
mod tests {
    ...
    fn test_state() -> crate::SharedState {
        crate::SharedState {
            raydium_price: Arc::new(Mutex::new(Some(rust_decimal::Decimal::from_f64_retain(150.0).unwrap()))),
            orca_price: Arc::new(Mutex::new(Some(rust_decimal::Decimal::from_f64_retain(150.0).unwrap()))),
            trades_today: Arc::new(Mutex::new(0)),
            profit_today: Arc::new(Mutex::new(rust_decimal::Decimal::from_f64_retain(0.0).unwrap())),
        }
    }
